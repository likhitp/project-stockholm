...
from chronology_generator import extract_event_details, generate_chronology_table, analyze_chronology
import re
import dateparser
import logging
    ...
    async def extract_events(self, document_text: str, document_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if json_start == -1 or json_end == 0:
                logger.warning(f"No JSON array found in response for {document_name}: '{response_text[:100]}...'")
                return []
            ...
            try:
                ...
                if not isinstance(events, list):
                    logger.warning(f"Parsed JSON is not a list for {document_name}, wrapping in list. Type: {type(events)}")
                ...
                for event in events:
                    if isinstance(event, dict):
                        ...
                        if standardized_event and standardized_event.get('description'):
                            ...
                            logger.warning(f"Skipping invalid or empty event structure in {document_name}: {event}")
                    else:
                        logger.warning(f"Skipping non-dict item in JSON array for {document_name}: {event}")
                
                logger.info(f"Successfully extracted and standardized {len(standardized_events)} events from {document_name}")
                ...
                logger.error(f"Type error during event processing for {document_name}: {str(te)}", exc_info=True)
                return []
                
        except Exception as e:
            logger.error(f"Unexpected error extracting events from {document_name}: {type(e).__name__} - {str(e)}", exc_info=True)
            return []

    async def create_chronology(self, events: List[Dict[str, Any]], case_description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            contents = [
                types.Content(
                    role="user",
                    parts=[
                        types.Part.from_text(text=f"Case Description:\n{case_description}\n\nEvents:\n{events_text}"),
                    ],
                ),
            ]
            ...
            if json_start != -1 and json_end != -1:
                ...
                if json_start != -1 and json_end != -1:
                    ...
                    return {
                        "events": events,
                        "analysis": analyze_chronology(events)
                    }
            ...
            try:
                ...
                for event in processed_events:
                    if isinstance(event, dict):
                        standardized_event = extract_event_details(event)
                        if standardized_event.get('description'):  # Only include events with descriptions
                            standardized_events.append(standardized_event)
