...
class TestIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_large_document_performance(self):
        ...
        self.assertLess(
            event_extraction_time, 30.0
        )  # Event extraction should take less than 30 seconds
        self.assertLess(
            chronology_generation_time, 10.0
        )  # Chronology generation should take less than 10 seconds
    ...
    async def test_error_handling(self):
        ...
        events = await self.llm_processor.extract_events(
            "This text contains no dates or events.", "no_dates.pdf"
        )
        self.assertEqual(len(events), 0)

        # Test with invalid date formats
        events = await self.llm_processor.extract_events(
            "Invalid date: 13/13/2024: Something happened", "invalid_dates.pdf"
        )
        chronology = await self.llm_processor.create_chronology(events, "Test case")
